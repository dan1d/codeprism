# codeprism Evaluation Suite

Evaluates codeprism search quality against a golden dataset of real ticket queries.

## Evaluating your own codebase

The default `golden_dataset.json` is tuned for the biobridge project. To evaluate
codeprism against **your own** indexed codebase, you have two options:

### Option A: Auto-generate from your codeprism index (recommended)

```bash
# Start codeprism with your indexed database, then:
...
The generator connects to your running codeprism instance, discovers all indexed flows
...
1. Replace `expected_flows` with flow names from your codeprism dashboard
...
```bash
# In a separate terminal, from the codeprism directory:
pnpm dev
# or via Docker:
docker run -p 4000:4000 codeprism
```
...
- **Flow Hit Rate** – what fraction of the *expected flows* appear in the returned cards. A flow hit means codeprism found the relevant area of the codebase.
